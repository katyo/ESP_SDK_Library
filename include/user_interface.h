...
uint64 system_mktime(uint32 year, uint32 mon, uint32 day, uint32 hour,
...
bool system_rtc_mem_write(uint8 des_addr, const void *src_addr,
			  uint16 save_size);
...
bool system_param_save_with_protect(uint16 start_sec, void *param,
				    uint16 len);
bool system_param_load(uint16 start_sec, uint16 offset, void *param,
		       uint16 len);
...
typedef void (*user_ie_manufacturer_recv_cb_t) (uint8 type, const uint8 sa[6],
						const uint8 m_oui[3],
						uint8 * ie, uint8 ie_len,
						int rssi);

bool wifi_set_user_ie(bool enable, uint8 * m_oui, uint8 type, uint8 * user_ie,
		      uint8 len);
