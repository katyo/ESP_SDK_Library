...
bool system_os_task(os_task_t task, uint8_t prio, os_event_t * queue,
		    uint8_t qlen);
...
bool system_rtc_mem_write(uint8_t des_addr, const void *src_addr,
			  uint16_t save_size);
...
bool system_param_save_with_protect(uint16_t start_sec, void *param,
				    uint16_t len);
bool system_param_load(uint16_t start_sec, uint16_t offset, void *param,
		       uint16_t len);
...
struct softap_config {
  uint8_t ssid[32];
  uint8_t password[64];		/* +32 */
  uint8_t ssid_len;		/* +96 Note: Recommend to set it according to your ssid */
  uint8_t channel;		/* +97 Note: support 1 ~ 13 */
  AUTH_MODE authmode;		/* +100 Note: Don't support AUTH_WEP in softAP mode. */
  uint8_t ssid_hidden;		/* +104 Note: default 0 */
  uint8_t max_connection;		/* +105 Note: default 4, max 4 */
  uint16_t beacon_interval;	/* +106 Note: support 100 ~ 60000 ms, default 100 */
};
...
typedef void (*user_ie_manufacturer_recv_cb_t) (uint8_t type, const uint8_t sa[6],
						const uint8_t m_oui[3],
						uint8_t * ie, uint8_t ie_len,
						int rssi);

bool wifi_set_user_ie(bool enable, uint8_t * m_oui, uint8_t type, uint8_t * user_ie,
		      uint8_t len);
int wifi_register_user_ie_manufacturer_recv_cb(user_ie_manufacturer_recv_cb_t
					       cb);
