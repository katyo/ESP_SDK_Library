...
/* ///////////////////////////////////////////////////////////////////////////////// */
static uint8_t *ICACHE_FLASH_ATTR
add_offer_options(uint8_t * optptr) {
...
/*	if(os_memcmp((char *)m->options,
  ...
  if ((m->options[0] == 0x63) && (m->options[1] == 0x82) &&
...
/*            addr_tmp.addr =  htonl(client_address_plus.addr);
    ...
    for (pback_node = plist; pback_node != NULL;
	 pback_node = pback_node->pnext) {
      ...
      if (os_memcmp(pdhcps_pool->mac, m->chaddr, sizeof(pdhcps_pool->mac)) ==
	  0) {
/*									os_printf("the same device request ip\n"); */
	if (os_memcmp
	    (&pdhcps_pool->ip.addr, m->ciaddr,
...
/*                                addr_tmp.addr = htonl(client_address.addr);
 ...
 POOL_CHECK:
    if ((client_address.addr > dhcps_lease.end_ip.addr) ||
	(client_address.addr == IPADDR_ANY)) {
...
wifi_softap_init_dhcps_lease(uint32 ip) {
  ...
  if (dhcps_lease_flag == TRUE) {	/* проверка */
	...
	|| dhcps_lease.end_ip.addr - dhcps_lease.start_ip.addr >
	(DHCPS_MAX_LEASE << 24)
  ...
  if (dhcps_lease_flag == FALSE) {	/* dhcps_lease не задавалась -> автоматическая установка */
    ...
    dhcps_lease.end_ip.addr =
      softap_ip | (local_ip + (DHCPS_MAX_LEASE << 24));
  }
#  if DHCPS_DEBUG
...
wifi_softap_set_dhcps_lease(struct dhcps_lease *please) {
  if (please == NULL ||
      (please->end_ip.addr - please->start_ip.addr > (DHCPS_MAX_LEASE << 24)))
    return false;
