...
static err_t
espconn_client_recv(void *arg, struct tcp_pcb *pcb, struct pbuf *p,
		    err_t err);
static void
  espconn_client_close(void *arg, struct tcp_pcb *pcb);

static err_t
espconn_server_recv(void *arg, struct tcp_pcb *pcb, struct pbuf *p,
		    err_t err);
static void
...
espconn_tcp_reconnect(void *arg) {
  ...
  if (precon_cb != NULL) {
	...
	espconn_printf("client: %d.%d.%d.%d : %d reconnection\n",
		       ...
		       espconn->proto.tcp->local_ip[1],
		       espconn->proto.tcp->local_ip[2],
		       espconn->proto.tcp->local_ip[3],
 ...
 *******************************************************************************/
void
espconn_tcp_disconnect(espconn_msg * pdiscon) {
  if (pdiscon != NULL) {
...
espconn_tcp_write(void *arg) {
  ...
  while (tcp_sndbuf(pcb) != 0) {
	...
	continue;
      ...
      err =
	espconn_tcp_sent(pwrite, pwrite->pcommon.ptail->punsent,
