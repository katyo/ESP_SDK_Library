...
#  include "gdbstub/gdbstub_config.h"
...
static long ATTR_GDBFN
gdbGetHexVal(unsigned char **ptr, int bits) {
  int i;
  int no;
  unsigned int v = 0;
  char c;
...
static unsigned char ATTR_GDBFN
readbyte(unsigned int p) {
  int *i = (int *)(p & (~3));

  /*      if (p<0x20000000 || p>=0x60000000) return -1; */
  if (p < 0x20000000 || p > 0x60009e00 || (p >= 0x60002000 && p < 0x60008000))
    return -1;
  return *i >> ((p & 3) * 8);
}
...
static void ATTR_GDBFN
writeByte(unsigned int p, unsigned char d) {
  int *i = (int *)(p & (~3));
...
#  endif
  ...
  if (gdbstub_savedRegs.reason == 0xff) {
    ...
    if (i < sizeof(exceptionSignal))
      return gdbPacketHex(exceptionSignal[i], 8);
    else
      gdbPacketHex(11, 8);
  } else {
...
static int ATTR_GDBFN
gdbHandleCommand(unsigned char *cmd, int len) {
  (void)len;
...
static unsigned int ATTR_GDBFN
getaregval(int reg) {
  if (reg == 0)
...
static void ATTR_GDBFN
setaregval(int reg, unsigned int val) {
  os_printf("%x -> %x\n", val, reg);
  if (reg == 0)
...
emulLdSt(void) {
  ...
  if ((i0 & 0xf) == 2 && (i1 & 0xf0) == 0x20) {
    ...
    *p = getaregval(i0 >> 4);
    ...
    os_printf("GDBSTUB: No l32i/s32i instruction: %x %x %x. Huh?", i2, i1,
              i0);
  }
}
...
install_exceptions(void) {
  ...
  int exno[] = {
    EXCCAUSE_ILLEGAL, EXCCAUSE_SYSCALL, EXCCAUSE_INSTR_ERROR,
    EXCCAUSE_LOAD_STORE_ERROR,
    EXCCAUSE_DIVIDE_BY_ZERO, EXCCAUSE_UNALIGNED, EXCCAUSE_INSTR_DATA_ERROR,
...
install_exceptions(void) {
  ...
  *ufe =
    ((((int)gdbstub_user_exception_entry -
       (int)user_fatal_exception_handler) - 4) << 6) | 6;
}
#  endif
...
uart_hdlr(void *arg, void *frame) {
  ...
  int doDebug = 0;
  int fifolen =
    (READ_PERI_REG(UART_STATUS(0)) >> UART_RXFIFO_CNT_S) & UART_RXFIFO_CNT;
  while (fifolen != 0) {
...
void ATTR_GDBFN
gdbstub_handle_uart_int(struct XTensa_rtos_int_frame_s *frame) {
