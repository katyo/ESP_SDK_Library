...
#ifdef USE_OVERLAP_MODE
  if (flash_read != NULL)
    return flash_read(flashchip, faddr, des, size);
#endif
  if (size != 0) {
	...
	if (blksize) {
      ...
      copy_s4d1((char *)des, (void *)(faddr + FLASH_BASE), size);
  }
...
spi_flash_real_size(void) {
  if (flash_size == 0) {
	...
	if (spi_flash_read(size, x2, 8 * 4) != SPI_FLASH_RESULT_OK)
	  break;
	else if (!ets_memcmp(x1, x2, 8 * 4))
	  break;
      };
    };
    flash_size = size;
...
spi_flash_write_bytes_array(uint32_t des_addr, uint8_t * src_addr, uint32_t size) {
  ...
  while (size) {
    if (size < 32) {
      xlen = size;
      if (xlen & 3)
	if (SPIRead(addr, (uint32_t *) & tmp.ui[xlen >> 2], 4) != 0) {
	  close_16m();
	  Cache_Read_Enable_def();
	  return ret;
	}
