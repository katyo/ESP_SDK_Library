...
eagle_lwip_if_alloc(struct ieee80211_conn *conn, const uint8 * macaddr,
  ...
  if (conn->dhcps_if == 0) {	/* +176 // SDK 1.4.0 +200 */
    ...
    myif->hostname = NULL;	/* +40 // +44 SDK 1.4.0 */
  ...
  if (conn->dhcps_if != 0) {	/* +176 // +200 SDK 1.4.0 */
    ...
    ets_task(task_if1, LWIP_IF1_PRIO, (ETSEvent *) lwip_if_queues[1],
	     QUEUE_LEN);
    netif_add(myif, &ipinfo->ip, &ipinfo->netmask, &ipinfo->gw, conn, init_fn,
	      ethernet_input);
    if (dhcps_flag) {
      dhcps_start(ipinfo);
...
#  endif
    ...
    netif_add(myif, &ipn.ip, &ipn.netmask, &ipn.gw, conn, init_fn,
	      ethernet_input);
  }
  return myif;
}
