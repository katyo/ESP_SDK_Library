...
#  endif
  ...
  asm volatile ("movi	a2, 1;" "slli   a1, a2, 30;");
...
sflash_something(uint32_t flash_speed) {
  ...
  if (flash_speed > 2) {	/* 80 MHz */
    ...
    value =
      1 + flash_speed + ((flash_speed + 1) << 8) + ((flash_speed >> 1) << 4);
...
read_align_exception_handler(struct exception_frame *ef, uint32_t cause) {
  ...
  asm("rsr   %0, EXCVADDR;"	/* read out the faulting address */
 ...
 : "=r"(excvaddr), "=r"(insn): "r"(epc1):"a4", "a5", "a6");
...
tst_cfg_wifi(void) {
  ...
  if (wifi_config->wfmode[0] == 0xff)
    wifi_config->wfmode[0] = SOFTAP_MODE;
  else
    wifi_config->wfmode[0] &= 3;
  ...
  if (wifi_config->field_311 > 2)
    wifi_config->field_311 = 0;
  if (wifi_config->field_312 > 8)
    wifi_config->field_312 = 4;
  ...
  if (wifi_config->field_169 > 2)
    wifi_config->field_169 = 0;	/* +169 */
  wifi_config->phy_mode &= 3;
  if (wifi_config->phy_mode == 0)
    wifi_config->phy_mode = 3;	/* phy_mode */
}
 ...
 */
  ...
  /*  */
  port_heap_init();		/* инициализация менеджера памяти heap */
  /*  */
  ...
  ets_timer_setfn(&check_timeouts_timer, (ETSTimerFunc *) sys_check_timeouts,
		  NULL);
#else
...
user_uart_wait_tx_fifo_empty(uint32_t uart_num, uint32_t x) {
  if (uart_num)
    ...
    while (((UART0_STATUS >> UART_TXFIFO_CNT_S) & UART_TXFIFO_CNT) && (x--));
}
